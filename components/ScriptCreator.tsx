...
export default function QuestCreator({ onSaveQuest }: QuestCreatorProps) {
  ...
  // Toggle lightning step (skip wait after this step)
  const handleToggleLightningStep = (index: number) => {
    setSteps((prev) =>
      prev.map((step, i) =>
        i === index ? { ...step, skipWait: !step.skipWait } : step
      )
    );
  };
  ...
  const handleSaveQuest = async () => {
    ...
    try {
      ...
      let script: QuestScript | undefined;
      if (steps.length > 0) {
        script = {
          id: questId,
          name: questName,
                  ...
                  >
                    ...
                    <label
                      style={{
                        ...
                        gap: '4px',
                        fontSize: '10px',
                        color: step.skipWait ? '#f59e0b' : '#999',
                        cursor: 'pointer',
                        whiteSpace: 'nowrap',
                      }}
                      title="Lightning step: Skip wait after this step"
                    >
                      <input
                        type="checkbox"
                        checked={step.skipWait || false}
                        onChange={() => handleToggleLightningStep(index)}
                        style={{
                          width: '12px',
                          height: '12px',
                          cursor: 'pointer',
